...
let main = async () => {
    ...
    async function registerRPC(method: string, plugin: string | null, input: Type, output: Type) {
        const reply = await callRPC<{method: string, plugin: string | null, inputMsg: string, outputMsg: string}, { assignedId: number }>(
            'RegisterRPC',
            {
                method,
                plugin,
    ...
    await registerRPC('GetBlockList', 'RemoteFortressReader', BlockRequest, BlockList)
    await registerRPC('ResetMapHashes', 'RemoteFortressReader', EmptyMessage, EmptyMessage)
    ...
    const blockReply = await callRPC(
        'GetBlockList',
        blockRequest
    )
